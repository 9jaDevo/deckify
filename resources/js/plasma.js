...
export function initPlasma(container, options = {}) {
    const { color = '#00FF7F', speed = 0.5, direction = 'forward', scale = 1, opacity = 1, mouseInteractive = true } = options;
    ...
    const program = new Program(gl, {
        ...
        uniforms: {
            ...
            uCustomColor: { value: new Float32Array(customColorRgb) }, uUseCustomColor: { value: useCustomColor },
            uSpeed: { value: speed * 0.4 }, uDirection: { value: directionMultiplier },
    ...
    const handleMouseMove = e => {
        ...
        program.uniforms.uMouse.value[0] = mousePos.x; program.uniforms.uMouse.value[1] = mousePos.y;
    };
    if (mouseInteractive) container.addEventListener('mousemove', handleMouseMove);
    const setSize = () => {
        const rect = container.getBoundingClientRect();
        renderer.setSize(Math.max(1, Math.floor(rect.width)), Math.max(1, Math.floor(rect.height)));
        program.uniforms.iResolution.value[0] = gl.drawingBufferWidth; program.uniforms.iResolution.value[1] = gl.drawingBufferHeight;
    };
    ...
    raf = requestAnimationFrame(loop);
    return () => { cancelAnimationFrame(raf); ro.disconnect(); container.removeEventListener('mousemove', handleMouseMove); container.removeChild(canvas); };
}
